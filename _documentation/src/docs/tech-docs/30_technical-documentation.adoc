...
For a given merge request event, 3 different independent actions can be made:
...
This means that when the auto-merge request finally gets merged, any other pending auto-merge requests between the same branches must still be merged.
...
Sometime the source branch of an auto merge-request is not deleted by gitlab. Ucascade makes sure that this branch gets deleted once the corresponding merge request is merged.
